...
//reads 1d csv files
std::vector<double> COR_predictor::read_csv1d(const char * filename)
{
	std::ifstream fin;
	double input;
...
//reads 2d csv files
std::vector<std::vector<double> > COR_predictor::read_csv2d(const char * filename)
{
	std::ifstream fin;
	double input;
...
void COR_predictor::write_csv1d(std::vector<double> a, const char * filename)
{
	std::ofstream fout;
	fout.open(filename);
	fout.precision(30);
...
void COR_predictor::write_csv2d(std::vector<std::vector<double> > a, const char * filename)
{
	std::ofstream fout;
	fout.open(filename);
	fout.precision(30);
...
void COR_predictor::Print_parameters(std::vector<std::vector<double> > param)
{
	int ni = param.size();
	int nj = param[0].size();
	for (int i=0; i<ni; ++i)
...
void COR_predictor::Write_parameters(std::vector<std::vector<double> > param)
{
	bool write = false;
