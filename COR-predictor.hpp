...
class cor
{
	...
	public:
		...
		class anneal
		{
			...
			parameters neighbor(parameters state0);
			std::vector<double> GetResiduals(std::vector<std::vector<double> > x_rand, parameters state);
			double SumOfSquares(std::vector<double> residuals);
			double lnfac(double n);
			double Entropy(parameters state, double energy);
			double Temperature(double delta_energy, double delta_entropy);
			public:
				void run();
		};
