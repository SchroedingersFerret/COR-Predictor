...
double cor::f(std::vector<double> x, std::vector<std::vector<double> > parameters)
{
	...
	y *= taylor(x[1],parameters[1]);
	y /= taylor(x[2],parameters[2]);
	y /= taylor(x[3],parameters[3]);
	y /= taylor(x[4],parameters[4]);
	y /= taylor(x[5],parameters[5]);
	y /= taylor(x[6],parameters[6]);
	return y;
}
...
{
	std::vector<std::bitset<64*nx> > w(nx);
	for (int i=0; i<nx; ++i)
	{
		for (int j=0; j<nx; ++j)
		{
...
{
	std::vector<std::vector<double> > parameters(nx,std::vector<double> (nx));
	for (int i=0; i<nx; ++i)
	{
		for (int j=0; j<nx; ++j)
		{
...
{
	std::vector<std::vector<std::bitset<64*nx> > > bin (n_initial, std::vector<std::bitset<64*nx> > (nx));
	std::vector<std::vector<double> > parameters (nx,std::vector<double> (nx));
...
void cor::genetic::radioactive_spider_semen()
{
	...
	{
		std::vector<std::vector<double> > parameters(nx,std::vector<double> (nx));
		int n_mutate = (int) (nx*nx*64*pm+1);
		for (int l=0; l<n_mutate; ++l)
		{
			int i_mutate = rand() % nx;
